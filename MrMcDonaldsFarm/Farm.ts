...
namespace Farm {
    ...
    class Animal {
        ...
        constructor(_type: string, _sound: string, _food: FoodSilo, _foodamount: number) {
            this.type = _type;
            this.sound = _sound;
            this.food = _food;
        ...
        checkFoodAmount(_foodSilos: FoodSilo[]): boolean {
            const foodSilo: FoodSilo | undefined = _foodSilos.find(_silo => _silo.type === this.food.type);
    ...
    const haySilo: FoodSilo = new FoodSilo("Hay", 11);
    const cornSilo: FoodSilo = new FoodSilo("Corn", 12);
    const meatSilo: FoodSilo = new FoodSilo("Meat", 42);
    const foodSilos: FoodSilo[] = [haySilo, cornSilo, meatSilo];
    ...
    for (let j: number = 0; j < foodSilos.length; j++) {
        foodSilos[j].restock();
    }
